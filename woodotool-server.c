...
wdt_mouse_get_name(sd_bus *bus,
                   ...
                   sd_bus_error *error)
{
    struct wdt_mouse *mouse = userdata;

    return sd_bus_message_append(reply, "s", mouse->name);
}

static int
wdt_mouse_move_relative(sd_bus_message *m,
                        void *userdata,
                        sd_bus_error *error)
{
    int x, y;
    ...
    printf("move relative: %d/%d\n", x, y);

    return sd_bus_reply_method_return(m, "");
}

static int
wdt_mouse_move_absolute(sd_bus_message *m,
                        void *userdata,
                        sd_bus_error *error)
{
    int x, y;
    ...
    printf("move absolute: %d/%d\n", x, y);

    return sd_bus_reply_method_return(m, "");
}

static int
wdt_mouse_press(sd_bus_message *m,
                void *userdata,
                sd_bus_error *error)
{
    unsigned int button;
    ...
    r = sd_bus_message_read(m, "u", &button);
    if (r < 0)
        return r;
...
static int
wdt_mouse_release(sd_bus_message *m,
                  void *userdata,
                  sd_bus_error *error)
{
    unsigned int button;
...
static int
wdt_mgr_get_mouse(sd_bus_message *m,
                  void *userdata,
                  sd_bus_error *error)
{
    struct wdt *ctx = userdata;
    struct wdt_mouse *mouse;
    char path[5];
    int r;
    pid_t pid = 0;
    sd_bus_creds *creds;
    ...
    sprintf(path, "%d", ctx->nmice);
    ctx->mice[ctx->nmice] = mouse;
    ctx->nmice++;

    asprintf(&mouse->name, "mouse-pid-%d-%s", pid, comm);

    r = sd_bus_path_encode(PATH_PREFIX "/mice",
                           path,
                           &mouse->path);

    printf("New mouse'%s' available at: %s\n", mouse->name, mouse->path);

    sd_bus_creds_unref(creds);

    return sd_bus_reply_method_return(m, "o", mouse->path);
}

static int
wdt_find_mouse(sd_bus *bus,
               const char *path,
               const char *interface,
               void *userdata,
               void **found,
               sd_bus_error *error)
{
    ...
    /* FIXME: return the correct mouse */
    *found = ctx->mice[0];
...
{
    struct wdt *ctx = userdata;
    char **paths = calloc(ctx->nmice + 1, sizeof(char *));
    int i;

    for (i = 0; i < ctx->nmice; i++)
        paths[i] = strdup(ctx->mice[i]->path);
...
const sd_bus_vtable wdt_mouse_vtable[] = {
    SD_BUS_VTABLE_START(0),
    SD_BUS_PROPERTY("Name", "s", wdt_mouse_get_name, 0, SD_BUS_VTABLE_PROPERTY_CONST),
    SD_BUS_METHOD("MoveRelative", "ii", "", wdt_mouse_move_relative, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("MoveAbsolute", "ii", "", wdt_mouse_move_absolute, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("Press", "u", "", wdt_mouse_press, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("Release", "u", "", wdt_mouse_release, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_VTABLE_END,
};
...
static const sd_bus_vtable wdt_mgr_vtable[] = {
    ...
    SD_BUS_METHOD("GetMouse", "", "o", wdt_mgr_get_mouse, SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_VTABLE_END,
};
